window.addEventListener("DOMContentLoaded", () => {
      ...
      data.length
        ? data.forEach((element) => {
            ...
            <div class="list-wrap">
                ...
                <h5 class="name">${element.mission_name} #${
              element.flight_number
            }</h5>
                <div class="text-small">Mission Ids <span> <ul>${
                  element.mission_id.length
                    ? element.mission_id.map((id) => `<li>${id}</li>`).join("")
                    : "<li>null</li>"
                }</ul></span></div>
                <div class="text-small">Launch Year <span>${
                  element.launch_year
                }</span></div>
                <div class="text-small">Successful Launch <span>${
                  element.launch_success
                }</span></div>
                <div class="text-small">Successful Landing <span>${
        ...
        : (loadList.innerHTML =
            "<p>No answer for selected filter from API or API server may be down</p>");
  ...
  function loadData(params1, params2, params3) {
    ...
    window.history.replaceState(
      null,
      null,
      `?launch_success=${params1}&land_success=${params2}&launch_year=${params3}`
    );
    fetch(
      `https://api.spaceXdata.com/v3/launches?limit=100&launch_success=${params1}&land_success=${params2}&launch_year=${params3}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  dateFilter.forEach((elem) => {
    elem.onclick = function (e) {
      ...
      launchFilter.forEach((el) => {
        if (el.checked) data1 = el.value;
      });

      var data2 = "";
      landFilter.forEach((el) => {
        if (el.checked) data2 = el.value;
      });
